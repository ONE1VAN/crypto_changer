$(document).ready((function() {
    function e(e, c, a) {
        "send" == a ? $.ajax({
            ...
            success: function(e) {
                e = e[c];
                let a = ($(".exchange__block-input-send").data("min") * e / 100 * bonus).toFixed(5) - 0,
                    t = ($(".exchange__block-input-send").data("max") * e / 100 * bonus).toFixed(5) - 0;
            ...
            success: function(e) {
                e = e[c];
                let a = ($(".exchange__block-input-receive").val() / bonus * 100 / e).toFixed(5) - 0;
            ...
            success: function(e) {
                e = e[c];
                let a = ($(".exchange__block-input-send").val() * e / 100 * bonus).toFixed(5) - 0;
    ...
    function l() {
        ...
        "SHIB" == n && (n = "BTC"), $.ajax({ url: "https://min-api.cryptocompare.com/data/price?fsym=" + n + "&tsyms=usdt", dataType: "json", success: function(a) { a = a.USDT, Value = parseFloat(Math.random() * (900 / a - 500 / a) + 500 / a).toFixed(2) + " " + n, $(".transactions__table-content").prepend('<tr class="transactions__tr" style="display: none;"><td class="transactions__td">' + e + '</td><td class="transactions__td">' + c + '</td><td class="transactions__td">' + t + '</td><td class="transactions__td">' + To + '</td><td class="transactions__td">' + Value + '</td><td class="transactions__td transactions__td_blue">Right now</td></tr>'), $(".transactions__table-content tr:first").fadeIn(), $(".transactions__table-content tr").length > 5 && $(".transactions__table-content tr:last").remove() } })
    }
