...
<table class="tbl">
	...
	<tr>
		...
		<th><b>Название</b></th>
		<th width="70"><b>Кошелек (Наш)</b></th>
	...
	function users_list($page, $num, $query)
	{
		...
		if (!$themes) {
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				print '
				<td>
					<a href="?a=edit_first_curr&id=' . $row['id'] . '">
			...
			if ($page != 1) $pervpage = "<a href=?a=first_curr&sort=" . $_GET['sort'] . "&page=" . ($page - 1) . ">««</a>";
			if ($page != $total) $nextpage = " <a href=?a=first_curr&sort=" . $_GET['sort'] . "&page=" . ($page + 1) . ">»»</a>";
			if ($page - 2 > 0) $page2left = " <a href=?a=first_curr&sort=" . $_GET['sort'] . "&page=" . ($page - 2) . ">" . ($page - 2) . "</a> | ";
			if ($page - 1 > 0) $page1left = " <a href=?a=first_curr&sort=" . $_GET['sort'] . "&page=" . ($page - 1) . ">" . ($page - 1) . "</a> | ";
			if ($page + 2 <= $total) $page2right = " | <a href=?a=first_curr&sort=" . $_GET['sort'] . "&page=" . ($page + 2) . ">" . ($page + 2) . "</a>";
			if ($page + 1 <= $total) $page1right = " | <a href=?a=first_curr&sort=" . $_GET['sort'] . "&page=" . ($page + 1) . ">" . ($page + 1) . "</a>";
