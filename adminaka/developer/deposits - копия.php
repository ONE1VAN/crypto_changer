        ...
        if($DepoActiv == 1){
          ...
          $plans = db_result_to_array(DB::Query("SELECT * FROM `plans` WHERE `id` = '".$plan."'"));
            ...
            $res = db_result_to_array(DB::Query('SELECT `id`, `login`, `point_balance`, `pm_balance`, `ref` FROM `users` WHERE `user` = "'.$login.'"'));
                    ...
                    if($plans[0]['back'] == 0){
                      $res = DB::Query("INSERT INTO `deposits` (`username`, `user`, `user_id`, `date`, `end`, `end_str`, `plan`, `sum`, `sum_bal`, `count`, `total_count`, `total_income`, `lastdate`, `nextdate`, `reinvest`, `bot`, `majak`, `visible`, `return_depo`, `ulog`, `pay`) VALUES ('".$email."', '".$login."', '".$id_user."', '".$date_startStr."', '".$end."', '".$endSTR."', '".$plan."', '".$amount."', '".$amountOutput."', '".$count."', '".$total_count."', '".$total_vip_amount."', '0', '".$nextdate."', '0', '1', '".$majak."', '".$visible."', '".$return."', '".$email."', '".$currency."')");
                    }else{
                      $res = DB::Query("INSERT INTO `deposits` (`username`, `user`, `user_id`, `date`, `end`, `end_str`, `plan`, `sum`, `sum_bal`, `count`, `total_count`, `total_income`, `lastdate`, `nextdate`, `reinvest`, `bot`, `majak`, `visible`, `return_depo`, `ulog`, `pay`) VALUES ('".$email."', '".$login."', '".$id_user."', '".$date_startStr."', '".$end."', '".$endSTR."', '".$plan."', '".$amount."', '".$amountOutput."', '".$count."', '".$total_count."', '".$total_amount."', '0', '".$nextdate."', '".$amount."', '1', '".$majak."', '".$visible."', '".$return."', '".$email."', '".$currency."')");
                    }
...
if($currency == 'usd'){
                ...
                DB::Query('UPDATE `users` SET `pm_balance` = `pm_balance` - "'.$amount.'" WHERE `id` = "'.$user_id.'" LIMIT 1');
            ...
            if($countlvl) {
              ...
              $ps    = ($amount / 100) * $lvlperc; // Сумма рефских
                 ...
                 $logins =   DB::Query("SELECT `login`, `ref_level`  FROM `users` WHERE `id` = '".$uref."'");
                    ...
                    if($urefp > 0) {
                      ...
                      DB::Query('UPDATE `users` SET `pm_balance` = "'.$new_pm_balance.'", `ref_money` = "'.$new_ref_balance.'"  WHERE `id` = "'.$uref.'" LIMIT 1');
                                      ...
                                      $user_info = db_result_to_array(DB::Query("SELECT `income_partners_all`, `income_partners_today` FROM `users` WHERE `login` = '".$loginR."'"));
                                        $income_partners_all = $user_info[0]['income_partners_all'] + $sum;
                                        $income_partners_today = $user_info[0]['income_partners_today'] + $sum;

                                        DB::Query ('UPDATE `users` SET `income_partners_usd` = `income_partners_usd` + "'.$sum.'" WHERE `login` = "'.$loginR.'" LIMIT 1');
                                            ...
                                            /*бонус за открытие новой реферальной линии при достижении нужного оборота*/
                                                        ...
                                                        DB::Query("UPDATE `income_usd` SET `income_partners_today` = '".$all_income_today."', `income_partners_all` = '".$all_income_all."' WHERE `login` = '".$loginR."'");
                                                            DB::Query("INSERT INTO `ref_stat` (`login`, `loginR`, `sum`, `ps`, `dateR`, `timeR`, `pay`) VALUES ('".$login."', '".$loginR."', '".$sum."', '".$ps."', '".$dateR."', '".$timeR."', '".$pay."')");
                        ...
                        $get_refp   =   DB::Query("SELECT `login`, `user`, `ref_percent`, `pm_balance`, `ref_money`, `ref_level` FROM `users` WHERE `login` = '".$_SESSION['user']."' LIMIT 1");
                                    $rowrefp            = mysqli_fetch_array($get_refp);
                                    $urefp              = $rowrefp['ref_percent'];
                                    $new_pm_balance     = $rowrefp['pm_balance'];
                                    ...
                                    $NewEmailUser       = $rowrefp['user']; 
                                                ...
                                                $res = db_result_to_array(DB::Query("SELECT `income_partners_today`, `income_partners_all` FROM `income_usd` WHERE `login` = '".$NewEmailLevel."'"));
                                                    $all_income_today = $res[0]['income_partners_today'] + $sum;
                                                    $all_income_all   = $res[0]['income_partners_all'] + $sum;

                                                $resP = db_result_to_array(DB::Query("SELECT `income_partners_today`, `income_partners_all` FROM `income_points` WHERE `login` = '".$NewEmailLevel."'"));
                                                    $all_income_todayP = $resP[0]['income_partners_today'] + $sum;
                                                    $all_income_allP   = $resP[0]['income_partners_all'] + $sum;
                       ...
                       /*бонус за открытие новой реферальной линии при достижении нужного оборота*/
                                                        $new_Ref_level = mysqli_fetch_assoc(DB::Query("SELECT * FROM `reflevels` WHERE `id` = '".$NewRefLevelId."'"));
                                                            ...
                                                            if(($income > $new_Ref_levels) || ($income == $new_Ref_levels)){
                                                              ...
                                                              DB::Query("UPDATE `users` SET `pm_balance` = '".$new_point_balance."' WHERE `login` = '".$NewEmailLevel."'");
                                                              DB::Query("UPDATE `users` SET `ref_level` = '".$NewRefLevelId."' WHERE `login` = '".$NewEmailLevel."'");
                                                              $title = 'Бонус за открытие реферального уровня';
                                                              DB::Query("INSERT INTO `bonus_history` (`login`, `email`, `amount`, `title`, `date`, `time`, `str`) VALUES ('".$NewEmailUser."', '".$NewEmailLevel."', '".$new_Ref_bonus."', '".$title."', '".$NewDateRef."', '".$NewTimeRef."', '".$NewStrRef."')");
                                                              DB::Query("UPDATE `income_points` SET `income_bonus_today` = `income_bonus_today`+ '".$new_Ref_bonus."', `income_bonus_all` = `income_bonus_all` + '".$new_Ref_bonus."' WHERE `login` = '".$loginR."'");
                                                            }
                   ...
                   DB::Query('UPDATE `users` SET `point_balance` = `point_balance` - "'.$sum.'" WHERE `id` = "'.$user_id.'" LIMIT 1');
          ...
          if($uref) {
            ...
            if($countlvl) {
              ...
              while($row =   mysqli_fetch_array($result)) {
                    if($i < $countlvl) {
              $lvlperc = $row['sum'];   // Процент уровня
              $ps    = ($sum / 100) * $lvlperc; // Сумма рефских
                 ...
                 $logins =   DB::Query("SELECT `login`, `ref_level`  FROM `users` WHERE `id` = '".$uref."'");
                           ...
                           if($i < $ref_level || $i == $ref_level){
                                    ...
                                    // Смотрим есть ли индивидуальный процент у данного реферала
                    $get_refp =   DB::Query("SELECT `login`, `user`, `ref_percent`, `point_balance`, `ref_point`, `ref_level` FROM `users` WHERE `id` = '".intval($uref)."' LIMIT 1");
                    $rowrefp  =   mysqli_fetch_array($get_refp);
                    $urefp    = $rowrefp['ref_percent'];
                    ...
                    if($urefp > 0) {
                      $ps = ($sum / 100) * $urefp; // Сумма рефских
                    }
                      ...
                      DB::Query('UPDATE `users` SET `point_balance` = "'.$new_pm_balance.'", `ref_point` = "'.$new_ref_balance.'"  WHERE `id` = "'.$uref.'" LIMIT 1');
                                      ...
                                      $user_info = db_result_to_array(DB::Query("SELECT `income_partners_all`, `income_partners_today` FROM `users` WHERE `id` = '".$uref."'"));
                                        $income_partners_all = $user_info[0]['income_partners_all'] + $sum;
                                        $income_partners_today = $user_info[0]['income_partners_today'] + $sum;
 
                                            DB::Query("UPDATE `users` SET `income_partners_all` = '".$income_partners_all."', `income_partners_today` = '".$income_partners_today."' WHERE `login` = '".$loginR."'");

                                            DB::Query ('UPDATE `users` SET `income_partners_point` = `income_partners_point` + "'.$sum.'" WHERE `login` = "'.$loginR.'" LIMIT 1');
                                                ...
                                                $resP = db_result_to_array(DB::Query("SELECT `income_partners_today`, `income_partners_all` FROM `income_points` WHERE `login` = '".$NewEmailLevel."'"));
                                                    ...
                                                    /*бонус за открытие новой реферальной линии при достижении нужного оборота*/
                                                        ...
                                                        /*бонус за открытие новой реферальной линии при достижении нужного оборота*/
                                                            ...
                                                            DB::Query("UPDATE `income_points` SET `income_partners_today` = '".$all_income_today."', `income_partners_all` = '".$all_income_allP."' WHERE `login` = '".$loginR."'");
                                                            DB::Query("INSERT INTO `ref_stat` (`login`, `loginR`, `sum`, `ps`, `dateR`, `timeR`, `pay`) VALUES ('".$login."', '".$loginR."', '".$sum."', '".$ps."', '".$dateR."', '".$timeR."', '".$pay."')");
                        ...
                        $uid    = $rowref['id'];
                         ...
                         $get_refp   =   DB::Query("SELECT `login`, `user`, `ref_percent`, `point_balance`, `ref_point`, `ref_level` FROM `users` WHERE `login` = '".$_SESSION['user']."' LIMIT 1");
                                    $rowrefp            = mysqli_fetch_array($get_refp);
                                    $urefp              = $rowrefp['ref_percent'];
                                    $new_ref_balance    = $rowrefp['ref_point'];
                                    $Ref_Level_True     = $rowrefp['ref_level'];
                         ...
                         $res = db_result_to_array(DB::Query("SELECT `income_partners_today`, `income_partners_all` FROM `income_points` WHERE `login` = '".$_SESSION['user']."'"));
                                    $all_income_all   = $res[0]['income_partners_all'];
                        $res = db_result_to_array(DB::Query("SELECT `income_partners_point`, `income_partners_usd` FROM `users` WHERE `login` = '".$loginR."'"));
                                                    $income_partners_usd     = $res[0]['income_partners_usd'];
                                                    $income_partners_point   = $res[0]['income_partners_point'];
                       ...
                       /*бонус за открытие новой реферальной линии при достижении нужного оборота*/
                                                        $new_Ref_level = mysqli_fetch_assoc(DB::Query("SELECT * FROM `reflevels` WHERE `id` = '".$NewRefLevelId."'"));
                                                            ...
                                                            if(($income > $new_Ref_levels) || ($income == $new_Ref_levels)){
                                                              ...
                                                              DB::Query("UPDATE `users` SET `point_balance` = '".$new_point_balance."' WHERE `login` = '".$NewEmailLevel."'");
                                                              DB::Query("UPDATE `users` SET `ref_level` = '".$NewRefLevelId."' WHERE `login` = '".$NewEmailLevel."'");
                                                              $title = 'Бонус за открытие реферального уровня';
                                                              DB::Query("INSERT INTO `bonus_history` (`login`, `email`, `amount`, `title`, `date`, `time`, `str`) VALUES ('".$NewEmailUser."', '".$NewEmailLevel."', '".$new_Ref_bonus."', '".$title."', '".$NewDateRef."', '".$NewTimeRef."', '".$NewStrRef."')");
